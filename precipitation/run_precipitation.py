...
from engine import train_one_epoch, validation_one_epoch
...
def get_args():
    parser = argparse.ArgumentParser('finetuning for precipitation prediction', add_help=False)
    ...
    parser.add_argument('--noise_weight', type=float, default=1.0,
                        help='noise_weight (default: 1.0)')
    parser.add_argument('--k_value', type=float, default=0.001,
                        help='k_value (default: 0.001)')
...
def main(args, ds_init):
    ...
    surf_vars=("2t", "10u", "10v", "msl", "tp")
    upper_vars = ("t", "u", "v", "r", "z")
    out_surf_vars = ["tp_12", "tp_24", "tp_36"]
    out_upper_vars = []
    
    data_path = "../datasets/0.25/china_organized"
    dataset_train = utils_data.Aurora_CDF_Dataset_china(startDate=args.train_start_date, endDate=args.train_end_date, horizon = args.horizon, num_points = len(out_surf_vars) + 2, nc_path=data_path)
    dataset_val = utils_data.Aurora_CDF_Dataset_china(startDate=args.val_start_date, endDate=args.val_end_date, evaluate=True, horizon = args.horizon, num_points = len(out_surf_vars) + 2, nc_path=data_path)

    
    static_vars_ds = xr.open_dataset("../aux_data/static.nc", engine="netcdf4")
    
    lat = torch.linspace(90, -90, 721)[lat_range[0]:lat_range[1]]
    lon = torch.linspace(0, 360, 1441)[:-1][long_range[0]:long_range[1]]
    level = (50, 100, 150, 200, 250, 300, 400, 500, 600, 700, 850, 925, 1000)
    static_vars = {
            # The static variables are constant, so we just get them for the first time.
            "lsm": torch.from_numpy(static_vars_ds["lsm"].values[0][lat_range[0]:lat_range[1],long_range[0]:long_range[1]]).float(),
            "z": torch.from_numpy(static_vars_ds["z"].values[0][lat_range[0]:lat_range[1],long_range[0]:long_range[1]]).float(),
            "slt": torch.from_numpy(static_vars_ds["slt"].values[0][lat_range[0]:lat_range[1],long_range[0]:long_range[1]]).float(),
        }
    ...
    in_channels = len(surf_vars)+len(upper_vars)*len(level)
    out_channels = len(out_surf_vars) + len(out_upper_vars)
    ...
    model = Aurora(use_ours=True, autocast=True,
                    ...
                    timestep = timedelta(hours=args.horizon),     
                    out_surf_vars = out_surf_vars,
                    out_atmos_vars = out_upper_vars,
                    task = "pre",
                    ours_prompt_length=20
                )
    ...
    if args.eval:
        ...
        test_stats = validation_one_epoch(data_loader_val, model, device,lat = lat, lon = lon, level = level, out_surf_vars = out_surf_vars,
                                            static_vars = static_vars, surf_vars=surf_vars, upper_vars=upper_vars, model_name=args.model, mode = args.mode)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            lat = lat, lon = lon, level = level, static_vars = static_vars,  model_name='Aurora', 
            surf_vars = surf_vars, upper_vars = upper_vars, out_surf_vars = out_surf_vars,
            use_ours=use_ours, total_step=args.epochs*num_training_steps_per_epoch, noise_weight=args.noise_weight, k_value=args.k_value
        )
